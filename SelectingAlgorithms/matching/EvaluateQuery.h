...
public:
    ...
    static size_t LFTJ(const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix, ui **candidates, ui *candidates_count,
                           ...
                           const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>>& order_constraints = {});
    // static std::tuple<size_t, double> exploreGraph(const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix,
    //     ui **candidates, ui *candidates_count, ui *order, ui *pivot,
    //     size_t output_limit_num, size_t &call_count,
    //     const std::unordered_map<unsigned int, std::pair<std::set<unsigned int>, std::set<unsigned int>>> &ordered_constraints);

    // static std::tuple<size_t, double> LFTJ(const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix,
    //     ui **candidates, ui *candidates_count, ui *order, size_t output_limit_num,
    //     size_t &call_count, size_t &valid_vtx_count,
    //     const std::unordered_map<unsigned int, std::pair<std::set<unsigned int>, std::set<unsigned int>>> &ordered_constraints);
    ...
    static size_t exploreKSSStyle(const Graph *query_graph, const Graph *data_graph, Edges***edge_matrix, ui **candidates, ui *candidates_count,
                                 ui *order, size_t output_limit_num, size_t &call_count, double &eltime,
                                 const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>>& order_constraints = {});
...
private:
    ...
    static size_t computeKSSEmbeddingNaiveImpl(ui depth, ui shell_num, ui* shell, ui** valid_cans, ui* valid_cans_count, bool * visited_v, std::chrono::steady_clock::time_point start_time); //変更
