...
{
    std::cout << "part50" << std::endl;
    VectorXd devalues;
    std::cout << "part51" << std::endl;
    VectorXd qevalues;
    std::cout << "part52" << std::endl;
    bool con = true;
    ...
    //std::cout << "part54" << std::endl;
        
    // for every C(q)
    ui vdata_vertex_num = 0;
    //std::cout << "part54.5" << std::endl;
    for (i = 0; i < qsiz; i++)
    { 
        
        label = query_graph->getVertexLabel(i);
        //std::cout << "part60" << std::endl;
        degree = query_graph->getVertexDegree(i);
        //std::cout << "part61" << std::endl;
        data_vertex_num = 0;
        //std::cout << "part62" << std::endl;
        for (j = 0; j < labelsNum[reverseLab[label]]; ++j)
        {
            // get Vertex ID
            VertexID data_vertex = labelData[reverseLab[label]][j];
            //std::cout << "part63" << std::endl;
            if (data_graph->getVertexDegree(data_vertex) >= degree)
            {
                ...
                if (con)
                {
                    ...
                    if (con)
                    {
                        //std::cout << "part71" << std::endl;
                        CSV cat(data_vertex);
                        //std::cout << "part72" << std::endl;
                        CS.emplace_back(cat);
                        //std::cout << "part73" << std::endl;
                    }
                }
            }
        }
        //std::cout << "part55" << std::endl;
        FCS.emplace_back(CS);
        //std::cout << "part56" << std::endl;
        CS.clear();
    }
}
...
int SpectralMatching(int sizd, Graph *data_graph, Graph *query_graph, int twohop, ui **&candidates, ui *&candidates_count, float **&EWeight, float **&eigenVD1, int alpha, int beta, Edges ***edge_matrix, float *&eigenQS)
{
    ...
    std::cout << "in Spectral matching" << std::endl;
    ui **candidates1 = NULL;
