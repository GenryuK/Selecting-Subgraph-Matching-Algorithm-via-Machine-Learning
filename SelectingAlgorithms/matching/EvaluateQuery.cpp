...
EvaluateQuery::exploreGraph(const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix, ui **candidates,
                            ui *candidates_count, ui *order, ui *pivot, size_t output_limit_num, size_t &call_count, double& eltime, 
    ...
    // std::cout << "cur_depth is" << std::endl;
    // std::cout << cur_depth << std::endl;

    // while (true) {
    //     // std::cout << "loop 1" << std::endl;
    //     while (idx[cur_depth] < idx_count[cur_depth]) { //このwhileの中でタイムアウトつけたい
    ...
    //             //output_limit_num -> 100000
    //             embedding_cnt += 1;
    //             visited_vertices[v] = false;
    //             std::cout << "embedding_cnt is " << embedding_cnt << std::endl;
    ...
    //             // std::cout << "after gene Candi id" << std::endl;
    ...
    //     cur_depth -= 1;
    //     std::cout << " cur_depth is " << cur_depth << std::endl;
    //     if (cur_depth < 0)
    //         break;
    //     else
    //         visited_vertices[embedding[order[cur_depth]]] = false;
    // }
    // auto start_time = std::chrono::steady_clock::now(); // 開始時間を記録
    ...
    while (idx[cur_depth] < idx_count[cur_depth]) { 
        clock_t now = clock();
        eltime = static_cast<double>(now - start) / CLOCKS_PER_SEC;
        if (eltime >= 10.0) {
        goto EXIT;
        }
        ...
        if (cur_depth == max_depth - 1) {
            ...
            idx[cur_depth] = 0;
            generateValidCandidateIndex(data_graph, cur_depth, embedding, idx_embedding, idx_count,
                                        valid_candidate_idx, edge_matrix, visited_vertices, bn, bn_count, order, pivot, candidates);
            pruneCandidatesIndexBySymmetryBreaking(cur_depth, embedding, order, candidates_count, candidates,
                                              idx_count, valid_candidate_idx, ordered_constraints);
        }
    }
    ...
    //return std::make_tuple(embedding_cnt, elapsed_time);
}

//--set time_out EXPLORE---------------------------------------------------------------------------------------------------
...
EvaluateQuery::LFTJ(const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix, ui **candidates,
                    ui *candidates_count,
                    ui *order, size_t output_limit_num, size_t &call_count, size_t &valid_vtx_cnt, double& eltime,
                    const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>>& ordered_constraints) {
...
#ifdef DISTRIBUTION
    if (embedding_cnt >= 10000000) { //output_limit_num -> 100000
...
size_t EvaluateQuery::exploreVEQStyle(const Graph *data_graph, const Graph *query_graph, TreeNode *tree,
                                    Edges ***edge_matrix, ui **candidates, ui *candidates_count,
                                    size_t output_limit_num, size_t &call_count, double& eltime, 
                                    const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>>& full_constraints) {
    clock_t start = clock();
    ...
    while (true) {
        while (idx[depth] < idx_count[depth]) {
            ...
            if (pi_m_index[u][idx[depth]] != (ui)-1) {
                ...
                // ui shortcuts_added = TM[u][equ_v_index];


                // if (depth == 0) {
                //     std::cout << "    (At the root, no previous matches)" << std::endl;
                // } else {
                //     for (ui i = 0; i < depth; ++i) {
                //         ui matched_query_vertex = order[i];
                //         ui matched_data_vertex = embedding[matched_query_vertex];
                //         std::cout << "    Query Vertex " << matched_query_vertex << " -> Data Vertex " << matched_data_vertex << std::endl;
                //     }
                // }
                // VertexID current_query_vtx = u; 
                // VertexID current_candidate_vtx = valid_cans[u][idx[depth]];
                // std::cout << "  Triggered while considering: Query " << current_query_vtx << " -> Data " << current_candidate_vtx << std::endl;

                
                // std::cout << "  Shortcut Count: " << shortcuts_added << " embeddings were added." << std::endl;
                // std::cout << "------------------------------------------" << std::endl;

                
                
                embedding_cnt += TM[u][equ_v_index];
                // std::cout << "count embedding1: " << embedding_cnt << std::endl;
                // std::cout << "  idx[depth]: " << idx[depth] << std::endl;
                // std::cout << "  idx_count[depth]: " << idx_count[depth] << std::endl;
                // for (ui i = 0; i < query_vertices_num; ++i) {
                //     std::cout << "  Query Vertex " << i << " -> Data Vertex " << embedding[i] << std::endl;
                // }
            ...
            if (depth == max_depth - 1) { 
                embedding_cnt += 1;
                // std::cout << "count embedding2: " << embedding_cnt << std::endl;
                // std::cout << "  idx[depth]: " << idx[depth] << std::endl;
                // std::cout << "  idx_count[depth]: " << idx_count[depth] << std::endl;
                // for (ui i = 0; i < query_vertices_num; ++i) {
                //     std::cout << "  Query Vertex " << i << " -> Data Vertex " << embedding[i] << std::endl;
...
ui EvaluateQuery::generateNextU(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count,
    ...
    for (ui i = 0; i < node.fn_count_; ++i) {
        ...
        if (extendable[u] == 0) { 
            ...
            // std::cout << "--- [Debug] Computed valid_cans for Query Vertex " << u << " ---" << std::endl;
            // std::cout << "  valid_cans_count[" << u << "]: " << valid_cans_count[u] << std::endl;

            // if (valid_cans_count[u] > 0) {
            //     std::cout << "  valid_cans[" << u << "]: ";
            //     for (ui k = 0; k < valid_cans_count[u]; ++k) {
            //         std::cout << valid_cans[u][k] << " ";
            //     }
            //     std::cout << std::endl;
            // }
            // else {
            //     std::cout << "  valid_cans[" << u << "]: None" << std::endl;
            // }
...
void EvaluateQuery::ComputeValidCans(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count,
    ...
    // std::cout << "\n============================================================" << std::endl;
    // std::cout << "[Debug] Enter ComputeValidCans for Query Vertex " << u << std::endl;
    // std::cout << "  Initial candidates_count[" << u << "]: " << candidates_count[u] << std::endl;
    // std::cout << "  Initial candidates[" << u << "]: ";
    // for (ui i = 0; i < candidates_count[u]; ++i) {
    //     std::cout << candidates[u][i] << " ";
    // }
    // std::cout << "\n--------------------------------------------------" << std::endl;
    // ----------------------------------------------------
    ...
    for (ui i = 0; i < candidates_count[u]; i++) {
        ...
        //std::cout << "  -> Testing candidate " << v << " for Query " << u << "..." << std::endl;
        // ------------------------------------------
        ...
        for (ui j = 0; j < unbrs_count; j++) {
            VertexID u_neighbor = unbrs[j];

            if (visited_u[u_neighbor] == true) {
                VertexID v_neighbor = embedding[u_neighbor];
                
                
                //std::cout << "     - Checking edge to " << v_neighbor << " (matched with Q-Vtx " << u_neighbor << ")... ";
                // -----------------------------------------------------------

                if (!data_graph->checkEdgeExistence(v, v_neighbor)) {
                    
                    //std::cout << "FAILED (Edge " << v << "-" << v_neighbor << " does not exist)." << std::endl;
                    // -----------------------------------------------------
                    flag = false;
                    break;
                } else {
        ...
        if (flag == true) {
            ...
            //std::cout << "  => Result: Candidate " << v << " is ACCEPTED." << std::endl;
            // -----------------------------------------
            valid_cans[u][valid_cans_count[u]++] = v;
        } else {
            
            //std::cout << "  => Result: Candidate " << v << " is REJECTED." << std::endl;
            // -----------------------------------------
        }
    }
...
EvaluateQuery::exploreKSSStyle(const Graph *query_graph, const Graph *data_graph, Edges***edge_matrix,
                               ...
                               size_t &call_count, double &eltime,
                               const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>>& order_constraints) {
    clock_t start = clock();
    size_t embedding_cnt = 0;
    int cur_depth = 0;
        ...
        while (idx[cur_depth] < idx_count[cur_depth]) {
            ...
            if (cur_depth == kernel_num - 1) {
                for(int i = 0; i < shell_num; i++) {
                    ...
                    clock_t now1 = clock();
                    eltime = static_cast<double>(now1 - start) / CLOCKS_PER_SEC;
                    if (eltime >= 10.0) {
                    goto EXIT;
                    }
...
EvaluateQuery::computeKSSEmbeddingNaive(ui shell_num, ui* shell, ui** valid_cans, ui* valid_cans_count, bool * visited_v) {
    ...
    return computeKSSEmbeddingNaiveImpl(0, shell_num, shell, valid_cans, valid_cans_count, visited_v, start_time2);
}

size_t 
EvaluateQuery::computeKSSEmbeddingNaiveImpl(ui depth, ui shell_num, ui* shell, ui** valid_cans, ui* valid_cans_count, bool * visited_v, std::chrono::steady_clock::time_point start_time2) {
    ...
    if (depth == shell_num - 1) {
        ...
        for(int i = 0; i < valid_cans_count[u_shell]; i++) {
            ...
            auto elapsed_time2 = std::chrono::duration<double>(now2 - start_time2).count();
            if (elapsed_time2 >= 10.0) {  
            std::cout << "Timeout reached after " << elapsed_time2 << " seconds, exiting loop." << std::endl;
            goto EXIT;
            }
        ...
        for(int i = 0; i < valid_cans_count[u_shell]; i++) {
            ...
            auto elapsed_time3 = std::chrono::duration<double>(now3 - start_time2).count();
            if (elapsed_time3 >= 10.0) {  
            std::cout << "Timeout reached after " << elapsed_time3 << " seconds, exiting loop." << std::endl;
            goto EXIT2;
            }
            VertexID v_id = valid_cans[u_shell][i];
            if(!visited_v[v_id]){
                visited_v[v_id] = true;
                embedding_cnt += computeKSSEmbeddingNaiveImpl(depth+1, shell_num, shell, valid_cans, valid_cans_count, visited_v, start_time2);
